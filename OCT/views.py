...
def dashboard_oc_tasks(request):
    ...
    if request.method == "POST":
        if request.POST['type'] == 'daily':
            ...
            new_daily_task = DailyTasks.objects.create(topic=request.POST['topic'],description=request.POST['description'],estimated_time=request.POST['estimated'],goal_related_to=goal)
...
def update_task(request):
    ...
    if request.method == 'GET':
        ...
        elif request.GET['type'] == 'daily':
            found_oct.daily_tasks.filter(topic=request.GET['name']).delete()
